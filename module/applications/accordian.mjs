...
export default class Accordion {
  constructor(config) {
    foundry.utils.logCompatibilityWarning(
      "The `Accordion` helper is deprecated in Mythic Realms. Use expanded sections via `ApplicationV2Mixin` instead.",
      { since: "MythicRealms 0.1", until: "MythicRealms 0.3" }
    );

    // Prevent double-wrapping previously processed content.
  ...
  bind(root) {
    ...
    for ( const heading of root.querySelectorAll(headingSelector) ) {
      ...
      // Cache the "full" expanded height (may update later on collapse).
  ...
  _onClickHeading(event) {
    // Allow anchors inside headings to behave normally.
  ...
  _onCollapseSection(heading, content, { animate=true }={}) {
    ...
    // Update cached height in case the content changed since last bind/expand.
